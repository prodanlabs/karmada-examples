...
func WaitEtcdReplicasetInDesired(c *kubernetes.Clientset, namespace, selector string, timeout int) error {
	...
	if err := wait.PollImmediate(time.Second, time.Duration(timeout)*time.Second, func() (done bool, err error) {
		...
		if int32(len(pods.Items)) == EtcdReplicas {
			klog.Infof("etcd desired replicaset is %v, currently: %v", EtcdReplicas, len(pods.Items))
			return true, nil
		}
		klog.Warningf("etcd desired replicaset is %v, currently: %v", EtcdReplicas, len(pods.Items))
		return false, nil
	}); err != nil {
		return err
	}
